...
import numpy as np
import torch
from .my_monotonic_align.core import maximum_path_c
print("My monotonic_align imported")
...
def maximum_path(value, mask):  
    ...
    # device = value.device
    # dtype = value.dtype
    # value = value.data.type(torch.float32).cpu().numpy()
    # path = np.zeros((value.shape[0], value.shape[1], value.shape[2]), dtype=np.int8)
    # mask = mask.data.cpu().numpy()

    # t_x_max = mask.sum(1)[:, 0].astype(np.int16)
    # t_y_max = mask.sum(2)[:, 0].astype(np.int16)
    
    # t_x = np.expand_dims(t_x_max, (1,2))
    # t_y = np.expand_dims(t_y_max, (1,2))
    # x_indices, y_indices = np.indices(value[0].shape, dtype=np.int16)
    # condition = (x_indices >= np.maximum(0, t_x - 2 * (t_y - y_indices) + 1)) & \
    #     (x_indices < np.minimum(t_x, 2 * y_indices + 1))
    # condition = ~condition
    # value[condition] = -1e09
