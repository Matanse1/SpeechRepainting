...
def main():
  ...
  with torch.no_grad():
    for batch_idx, data in tqdm(enumerate(val_loader)):
      ...
      #saving results
      save_dir = Path(os.path.join(model_dir, "alignment_results_with-mask", cp_num, f"sample_{batch_idx}")) 
      ...
      for i in range(len(full_phoneme_squence)):
        full_phoneme_squence[i] = phoneme_dict_d2p[full_phoneme_squence[i]]
