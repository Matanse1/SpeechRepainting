from transformers import AutoFeatureExtractor, WavLMModel, AutoModel
import torch
...
# config = model.config
# print(config.return_attention_mask)
...
spk1_ut1_r = torch.randn(16, 16000 * 30).cuda()
# spk2_ut1_r = torch.randn(16, 16000 * 20).cuda()

mask = tensor = torch.cat([torch.ones(16, 16000 * 15), torch.zeros(16, 16000 * 15)],  dim=-1).cuda()
# batch = torch.stack((spk1_ut1_r, spk2_ut1_r), dim=0)
with torch.no_grad():
    # batch_output = model(batch) #[B, T, fixed_dim]
    # reverb_audio1_pad = F.pad(reverb_audio1, ((240, 0)), "constant", 0)
    reverb_audio1_out = model(spk1_ut1_r, attention_mask=mask).last_hidden_state
