...
class VarianceAdaptor(nn.Module):
    ...
    def forward(self, x, src_mask, masked_frame_number=None, mel_len=None, mel_mask=None, 
        ...
        if duration_target is not None:
            ...
            mel_mask = utils.get_mask_from_lengths(mel_len)
            # duration_rounded = torch.clamp(torch.round(torch.exp(log_duration_prediction)-1.0), min=0)  *4
            # total_duration = torch.sum(duration_rounded, dim=-1, keepdim=True)# [B, 1]
            # mask = total_duration > masked_frame_number 
            # if mask.any():
            #     print(f"Duration is larger than masked_frame_number in {duration_rounded - masked_frame_number}")
            #     masked_est_ratio = masked_frame_number / total_duration #[B, 1]
            #     duration_rounded = torch.where(mask, torch.floor(duration_rounded * masked_est_ratio), duration_rounded)  # apply cutting ratio for specific samples in the batch
            #     total_duration = torch.sum(duration_rounded, dim=-1, keepdim=True)# [B, 1]
