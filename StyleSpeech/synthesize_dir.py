# This file synthesizes speech using the StyleSpeech model for a folder that contains the masked_audio_time and text
import torch
import numpy as np
import os
...
def synthesize(args, model, _stft):   
    ...
    for sample_folder in sample_folders:
        sample_num = Path(sample_folder).name
        os.makedirs(os.path.join(args.save_path, sample_num), exist_ok=True)
        ref_audio = os.path.join(sample_folder, 'masked_audio_time.wav')
        path_text = os.path.join(sample_folder, 'asr_text.txt')
        with open(path_text, 'r') as f:
            lines = f.readlines()
            text_guidance = lines[0].strip()
            text_guidance = re.sub(r'^asr_generated_signal:\s+', '', text_guidance)
            
        ref_mel = preprocess_audio(ref_audio, _stft).transpose(0,1).unsqueeze(0)
        src = preprocess_english(text_guidance, args.lexicon_path, with_space=args.with_space).unsqueeze(0)
        ...
        if args.with_space:
            ...
            sf.write(os.path.join(save_path, sample_num, 'generated_audio_hifi_gan.wav'), wav, 16000)
        else:
            wav = vocoder.inverse(mel_output.transpose(-2, -1)).squeeze().cpu().detach().numpy()
            sf.write(os.path.join(save_path, sample_num, 'generated_audio_hifi_gan.wav'), wav, 16000)
        copy_gt_path = os.path.join(sample_folder, 'gt_audio_hifi_gan.wav')
        if os.path.exists(copy_gt_path):
            os.makedirs(os.path.join(save_path, sample_num), exist_ok=True)
            shutil.copyfile(copy_gt_path, os.path.join(save_path, sample_num, 'gt_audio_hifi_gan.wav'))
        copy_gt_path = os.path.join(sample_folder, 'masked_audio_time.wav')
        if os.path.exists(copy_gt_path):
            os.makedirs(os.path.join(save_path, sample_num), exist_ok=True)
            shutil.copyfile(copy_gt_path, os.path.join(save_path, sample_num, 'masked_audio_time.wav'))
        copy_gt_path = os.path.join(sample_folder, 'asr_text.txt')
        if os.path.exists(copy_gt_path):
            os.makedirs(os.path.join(save_path, sample_num), exist_ok=True)
            shutil.copyfile(copy_gt_path, os.path.join(save_path, sample_num, 'asr_text.txt'))
        print('Generate done!')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default='/home/dsi/moradim/SpeechRepainting/StyleSpeech/results_greater_7_gap=100/')
