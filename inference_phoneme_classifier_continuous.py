...
def calc_phoneme(net, loss_fn, melspec, masked_cond, mask, phoneme_target, diffusion_cfg, diffusion_hyperparams, 
    ...
    temp = train_class(sde, loss_fn)
    temp.load_data(dict(melspec_key=melspec, masked_cond_key=masked_cond, mask_key=mask, mask_mask_key=mask_mask, masked_audio_time_mask_key=masked_audio_time_mask
                , text_key = None, input_text_key=None, melspec_mask_key=None, masked_melspec_mask_key=None))
    x_t = temp.prepare_step()
    t = temp._t
    B = temp._B
    if on_noisy_masked_melspec:
            x_t = temp.apply_mask(x_t, mask)
    ...
    if (not phoneme_target_mask) or (not mask_mask) or (not masked_audio_time_mask):
        ...
        weighted_loss = (1 - w_masked_pix) * unmaksed_loss + w_masked_pix * masked_loss
    return weighted_loss, phoneme_estimated, t, x_t
...
def inference(
    ...
    phoneme_dict_path = "/home/dsi/sellama/SpeechRepainting/phones.txt"
    _, phoneme_dict_d2p = get_phones_dict(phoneme_dict_path)

    # map diffusion hyperparameters to gpu
    diffusion_hyperparams  = get_diffusion_hyperparams(diffusion_cfg, fast=True)  # dictionary of all diffusion hyperparameters
    ...
    with torch.no_grad():
        for i in tqdm(range(n_samples_test)):
            ...
            weighted_loss, phoneme_estimated, diffusion_steps, transformed_X = calc_phoneme(net, criterion, gt_melspec, masked_cond, mask, phoneme_target, diffusion_cfg,
            ...
            with open(phoneme_filename, 'w') as f:
                f.write("<html><body>\n")
                f.write(f"<h1 style='text-align: center;'>The step is: {diffusion_steps.item()}/{diffusion_cfg.VPSDE.N}</h1>\n")  # Add a title
