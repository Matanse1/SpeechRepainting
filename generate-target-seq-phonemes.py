...
if __name__ == "__main__":
    ...
    style_speech_model, config = get_StyleSpeech(style_speech_config_path, style_speech_ch_path) # eval() to the model is already occuring here
    style_speech_model = style_speech_model.to(device)
    stft = TacotronSTFT(
    ...
    line = lines[1]
    match = re.search(r"sr_condition\s+: (.+)", line)
    if match:
    ...
    ref_masked_mel, _ = stft.mel_spectrogram(torch.tensor(masked_audio_time, dtype=torch.float32).unsqueeze(0)) #[B, F, T1]
    # Save the ref_masked_mel as an image using matplotlib
    # import matplotlib.pyplot as plt

    # ref_masked_mel_image = ref_masked_mel.squeeze(0).cpu().numpy()
    # plt.figure(figsize=(10, 4))
    # plt.imshow(ref_masked_mel_image, aspect='auto', origin='lower', cmap='viridis')
    # plt.colorbar(format='%+2.0f dB')
    # plt.title("Ref Masked Mel Spectrogram")
    # plt.xlabel("Time")
    # plt.ylabel("Mel Frequency")
    # plt.tight_layout()
    # plt.savefig("ref_masked_mel_image.png")
    # plt.close()
    with torch.no_grad():
        # Extract style vector
        ref_masked_mel = ref_masked_mel.to(device)
        style_vector = style_speech_model.get_style_vector(ref_masked_mel.transpose(1, 2)) # the input ref_masked_mel is [B, T, F], style_vector shape is [B, D=128]
        log_duration_prediction = style_speech_model.get_duration(style_vector, input_text, phoneme_length, masked_frame_number=None)
    print("Duration prediction: ", torch.exp(log_duration_prediction))
    print("The est duration is: ", torch.exp(log_duration_prediction).sum())
    print("The true duration is: ", ref_masked_mel.shape[-1])"""
    ...
    save_logp_attn = True
    save_dir_root = "/home/dsi/moradim/SpeechRepainting/"
    ...
    generator = models.FlowGenerator(
        n_vocab=num_symbol + getattr(hps.data, "add_blank", False) + getattr(hps.data, "without_blank_with_silence", False), 
    ...
    with torch.no_grad():
        for batch_idx, data in tqdm(enumerate(val_loader)):
            ...
            = targets_length_original[0], targets_length_original[1], targets_length_original[2], targets_length_original[3]
                ...
                #saving results
            save_dir = Path(os.path.join(save_dir_root, "alignment_results", cp_num, f"sample_{batch_idx}")) 
