    ...
    while n_iter < n_iters + 1:
        ...
        # for data in tqdm(trainloader, desc=f'Epoch {n_iter // len(trainloader)}'):
            ...
            if n_iter % iters_per_logging == 0 and rank == 0:
                ...
                writer.add_scalar("train_loss_step", reduced_loss, n_iter)
                writer.flush()
...
def training_loss(net, loss_fn, melspec, masked_cond, mask, mask_mask,
                  diffusion_hyperparams, text, input_text,
                  masked_audio_time_mask, on_noisy_masked_melspec,
                  w_masked_pix=0.7, sde=None):
    """
...
#                             mask_padding_time=masked_audio_time_mask, mask_padding_frames=mask_mask)
...
#         loss = loss_fn(epsilon_theta, z)
    ...
    loss = loss * mask_mask
    unmasked_loss = torch.sum(mask * loss) / (torch.sum(mask * mask_mask) * loss.shape[1])
    masked_loss   = torch.sum((1-mask) * loss) / (torch.sum((1-mask) * mask_mask) * loss.shape[1])
    return (1 - w_masked_pix) * unmasked_loss + w_masked_pix * masked_loss


def test_loss(net, loss_fn, melspec, masked_cond, mask, mask_mask,
              diffusion_hyperparams, text, input_text,
              masked_audio_time_mask, on_noisy_masked_melspec,
              w_masked_pix=0.7, sde=None):
    return training_loss(net, loss_fn, melspec, masked_cond, mask, mask_mask,
                         diffusion_hyperparams, text, input_text,
                         masked_audio_time_mask, on_noisy_masked_melspec,
