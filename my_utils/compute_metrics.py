...
class Metrics:
    ...
    def compute_metrics(self, pred, tar_wav, target_text):
        ...
        waveform = whisper.pad_or_trim(pred.flatten()).to(pred.device)
        mel = whisper.log_mel_spectrogram(waveform)
        hypo = self.whisper_model.decode(mel, self.options).text
        hypo_clean = self.normalizer(hypo)
        reference_clean = self.normalizer(target_text)
...
if __name__ == '__main__':
    ...
    path2data = '/dsi/gannot-lab1/users/mordehay/speech_repainting/exp/LibSp_wavlm-base-plus-rep_w_masked_pix=0.8_two_branch=True_specific_hidden_states/wnet_h512_d12_T400_betaT0.02/as-train-gap_asr_guidance_80cp/w1=2_w2=1.5_asr_start=270'
    ...
    titles = titles + [met + '_' + voc for met in ['plcmos_target_init', 'LSD_init', 'STOI_init', 'PESQ_init'] for voc in vocoder_names] + \
                [met + '_' + voc for met in ['WER', 'plcmos_pred', 'LSD', 'STOI', 'PESQ'] for voc in vocoder_names]
    compute_metrics = Metrics()
    dict_row_results = {}
    ...
    with open(save_csv_path, mode="w", newline="") as file:
        ...
        for sample in range(num_dirs):
            with open(f'{path2data}/sample_{sample}/asr_text.txt', "r") as file:
                lines = file.readlines()
                # Extract the sentences
                target_text = lines[0].split(":")[1].strip()
                est_text = lines[1].split(":")[1].strip()
            masked_audio, sr = torchaudio.load(f'{path2data}/sample_{sample}/time_masking_audio.wav') #masked_audio_time
            masked_audio = masked_audio[0].to(device)
            dict_row_results['masked_WER'] = compute_metrics.calc_wer(target_text, masked_audio)
...
input_csv = f"{path2data}/results.csv" #samples_info
