...
with open(input_text_path, 'rb') as file:
    ...
    input_text = [x for x in input_text if x != 'space'] # remove space tokens if they exist, since the model was trained without them
...
diffusion_cfg = DotDict({
    ...
    "VPSDE": DotDict({"beta_min":  0.1, "beta_max": 20, "N": 1000}),
    "VESDE": DotDict({"sigma_min": 0.01, "sigma_max": 50.0, "N": 1000}),
})

diffusion_hyperparams = get_diffusion_hyperparams(diffusion_cfg, fast=False)
if diffusion_hyperparams["name"] == "VPSDE":
    sde = VPSDE(
        diffusion_hyperparams["beta_min"],
    ...
    sde = VESDE(
        diffusion_hyperparams["sigma_min"],
...
mean, std = sde.marginal_prob(mel, None, continuous_t)
mel = mean + std[:, None, None] * z  # compute x_t from the continuous SDE marginal q(x_t|x_0)
...
asr_guidance_net = nnet.AudioEfficientConformerInterCTC(vocab_size=vocab_size, att_type=att_type, interctc_blocks=interctc_blocks, strides_subsampling=1)
